...
var tokenize = (function() {
    ...
    function getFormatter(factory, path, format, options) {
        return (!path ? factory : 
            function(input) {
    ...
    return function(type, path, alignment, format, options) {
        var formatter = getFormatter(options.formatterFactory(type, format, options), (path && path.split(sep)), format, options);
        if (!alignment) {
            return formatter;
        } else {
...
var compile = (function() {
    ...
    return function(template, options) {
        ...
        var compiledTemplate = template.replace(re, function(match, path1, type, path2, padding, format) {
            var path;
            if (match === '@@')
                return '@';
