...
var tokenize = (function() {
    ...
    function getFormatter(path, formatterFactory) {
        function format(input) { return formatterFactory(input)(input); }
        return (!path ? format : 
            function(input) {
                return format(isUndefined(input) ? input : path.reduce(pathReduce, input));
            }
        );
    }
    
    return function(path, alignment, format, options) {
        var formatter = getFormatter((path && path.split(sep)), options.valueFactory(format, options));
        if (!alignment) {
            return formatter;
        } else {
            alignment = parseAlignment(alignment);
            return function(input){ return align(formatter(input), alignment); };
        }
    };
}());
 ...
 ******************************************************************************/
/**
 * Default formatter factory for values. Delegates formatter to used based on
 * runtime type of object.
 * 
 * @param [format] Format option string
 * @param [options] SSF options captured for this formatter.
 */
var formatterForValue = function(format, options) {
    var fu, fn, fs, fd, fa, fo;
    return function(v) {
        if (isUndefined(v))     return (fu || (fu = options.undefinedFactory(format)));
        else if (isNumber(v))   return (fn || (fn = options.numberFactory(format)));
        else if (isString(v))   return (fs || (fs = options.stringFactory(format)));
        else if (isArray(v))    return (fa || (fa = options.arrayFactory(format)));
        else if (isDate(v))     return (fd || (fd = options.dateFactory(format)));
        else                    return (fo || (fo = options.objectFactory(format)));
    };
};
 ...
 ******************************************************************************/
var defaults = {
    'valueFactory': formatterForValue,
